import { createContext, useCallback, useContext, useEffect, useLayoutEffect, useMemo, useRef, useState, Fragment, type ChangeEvent, type FormEvent, type PointerEvent as ReactPointerEvent, type ReactNode } from 'react';
...
const homeFeedHeaderCopy: Record<HomeFeedTab, { en: string; ar: string; enSub: string; arSub: string }> = {
  'for-you': { en: 'For you', ar: 'لك', enSub: 'People, places and edits matched to your taste.', arSub: 'أشخاص وأماكن وتعديلات تناسب ذوقك.' },
  following: { en: 'Following', ar: 'تتابعهم', enSub: 'Updates from people you follow.', arSub: 'تحديثات من الأشخاص الذين تتابعهم.' },
...
type SavedList = { id: string; name: string; editIds: string[]; createdAt: string; updatedAt: string };
...
function TastekinApp() {
  ...
  const { data: circleMemberStatus } = useGetCircleMemberStatus(
    publicCreatorProfile?.username || '',
    { query: { enabled: myCircleEnabled && session.status === 'authenticated' && !!publicCreatorProfile?.username, queryKey: getGetCircleMemberStatusQueryKey(publicCreatorProfile?.username || '') } }
  );
  ...
  const loadSavedLists = useCallback(async () => {
    if (session.status !== 'authenticated') { setSavedLists([]); setActiveSavedListId(null); return; }
    try {
      const response = await fetch('/api/me/saved-lists', { credentials: 'include', cache: 'no-store' });
      setSavedLists(response.ok ? await response.json() as SavedList[] : []);
    } catch { setSavedLists([]); }
  }, [session.status]);
  useEffect(() => { void loadSavedLists(); }, [loadSavedLists, session.revision]);
  const createSavedList = async (name: string) => {
    const response = await fetch('/api/me/saved-lists', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) });
  ...
  const updateSavedListItem = async (listId: string, editId: string, active: boolean) => {
    const previous = savedLists;
    setSavedLists((current) => current.map((list) => list.id === listId ? { ...list, editIds: active ? Array.from(new Set([...list.editIds, editId])) : list.editIds.filter((id) => id !== editId) } : list));
    try {
      const response = await fetch(`/api/me/saved-lists/${encodeURIComponent(listId)}/edits/${encodeURIComponent(editId)}`, { method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ active }) });
      if (!response.ok) throw new Error(await describeFailedResponse(response));
    } catch (error) {
  ...
  const { data: circleMembers = [], isLoading: circleMembersLoading, error: circleMembersError, refetch: refetchCircleMembers } = useListCircleMembers({
    query: {
      ...
      queryKey: getListCircleMembersQueryKey()
    }
  });
  const { data: circleFeedItems = [], isLoading: circleFeedLoading, error: circleFeedError, refetch: refetchCircleFeed } = useGetCircleFeed({
    query: {
      enabled: myCircleEnabled && session.status === 'authenticated' && homeFeedTab === 'my-circle',
      queryKey: getGetCircleFeedQueryKey()
    }
  });

  const circleFeedEdits: CreatorEdit[] = useMemo(() => circleFeedItems.map(item => {
    const member = circleMembers.find((m) => m.username === item.creatorUsername);
    return {
      ...item.edit,
      creatorUsername: item.creatorUsername,
      creatorName: member?.displayName || item.creatorName,
      creatorVerified: item.creatorVerified,
      creatorAvatar: member?.avatar,
      following: true
    } as CreatorEdit;
  ...
  const toggleSaved = async (id: string) => {
    if (session.status !== 'authenticated') { go('auth'); return; }
  ...
  useEffect(() => {
    if (screen === 'add' && owner) openComposer();
    // The legacy Creator Workspace screen is retained, but creator visits redirect
    // directly into the existing composer.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [screen, owner]);
  ...
  const abandonComposer = () => go('you');
  const finishSavedCreatorFlow = () => { pendingMediaIsDiscardable.current = false; setPendingMediaPaths([]); setScreen('you'); };
  ...
  return <TasteSessionContext.Provider value={session}><div className="approved-app" dir={ar ? 'rtl' : 'ltr'}><main className="approved-shell">
    ...
    {screen === 'home' && <><section className="home-feed-header"><h1>{ar ? homeFeedHeaderCopy[homeFeedTab].ar : homeFeedHeaderCopy[homeFeedTab].en}</h1><p>{ar ? homeFeedHeaderCopy[homeFeedTab].arSub : homeFeedHeaderCopy[homeFeedTab].enSub}</p></section><HomeFeedTabs ar={ar} active={homeFeedTab} onSelect={setHomeFeedTab} />
      {homeFeedTab === 'my-circle' && !circleLoading && !circleError && circleMembers.length > 0 && (
        <>
          <div className="circle-rail" aria-label={ar ? 'أعضاء دائرتك' : 'Your circle members'} role="list">
            {circleMembers.map((member) => (
              <button key={member.creatorId} className="circle-rail-member" role="listitem" data-testid={`circle-member-${member.username}`} onClick={() => { setSelectedCreatorUsername(member.username); go('profile'); }} aria-label={ar ? `عرض ملف ${member.displayName}` : `View ${member.displayName}'s profile`}>
                <Avatar profile={{ avatar: member.avatar, displayName: member.displayName } as any} />
...
function HomeFeedTabs({ ar, active, onSelect }: { ar: boolean; active: HomeFeedTab; onSelect: (tab: HomeFeedTab) => void }) {
  ...
  const tabs: Array<{ id: HomeFeedTab; en: string; ar: string }> = [{ id: 'for-you', en: 'For You', ar: 'لك' }, { id: 'following', en: 'Following', ar: 'تتابعهم' }];
  if (session.status === 'authenticated' && session.featureFlags.my_circle === true) tabs.push({ id: 'my-circle', en: 'My Circle', ar: 'دائرتي' });
...
function FeedEmpty({ ar, tab, onExplore }: { ar: boolean; tab: Exclude<HomeFeedTab, 'for-you'>; onExplore: () => void }) {
  const copy = tab === 'following'
    ...
    : (ar ? ['لا توجد تعديلات في دائرتك بعد.', 'أضف مبدعين موثقين لبناء مساحتك الخاصة.'] : ['No edits in your circle yet.', 'Add verified creators to build your private space.']);
...
function Avatar({ profile = defaultCreatorProfile, src }: { profile?: CreatorProfile; src?: string }) {
    ...
    <section>
      ...
      {session.status === 'signed-out' && sort === 'best' && (
         <div className="workspace-notice">
            ...
            <button onClick={onSignIn}>{t('Sign in', 'تسجيل الدخول')}</button>
         </div>
      )}
...
function TuneTasteScreen({ ar, onBack, onSignIn }: { ar: boolean; onBack: () => void; onSignIn: () => void }) {
    ...
    return (
      <SimpleScreen kicker={ar ? 'تطابق الذوق' : 'Taste Match'} title={ar ? 'ضبط ذوقك' : 'Tune your taste'}>
        <div className="workspace-notice" role="alert">
          ...
          <button onClick={onSignIn}>{ar ? 'تسجيل الدخول' : 'Sign in'}</button>
        </div>
...
function ReportMenu({ ar, targetType, targetId, onSignIn, label, blockUsername, onBlocked, muteUsername, onViewPublicProfile, showReport = true }: { ar: boolean; targetType: 'edit' | 'comment' | 'profile'; targetId: string; onSignIn: () => void; label?: string; blockUsername?: string; onBlocked?: () => void; muteUsername?: string; onViewPublicProfile?: () => void; showReport?: boolean }) {
  ...
  const block = async () => {
    if (!blockUsername) return;
    setStep('block-submitting');
    try {
    ...
    <Drawer.Root open={open} onOpenChange={(next) => { setOpen(next); if (!next) reset(); }}>
      <Drawer.Portal>
        ...
        <Drawer.Content className="approved-drawer-content report-drawer" aria-label={ar ? 'الإبلاغ' : 'Report'}>
          ...
          {step === 'block-confirm' && <div className="report-status">
            <h2 className="approved-title" style={{ margin: '0 0 12px', fontSize: 18 }}>{ar ? 'حظر هذا المستخدم؟' : 'Block this user?'}</h2>
            <p>{ar ? 'لن تتمكنا بعد الآن من رؤية ملفكما الشخصي أو التفاعل مع بعضكما البعض. يمكنك إلغاء الحظر لاحقاً من الإعدادات.' : 'You won’t be able to see each other’s profiles or interact anymore. You can unblock this account later from Settings.'}</p>
            <div className="admin-confirm-actions">
              <button type="button" className="approved-button" onClick={() => setStep('menu')}>{ar ? 'إلغاء' : 'Cancel'}</button>
              <button type="button" className="approved-button primary" onClick={() => void block()}>{ar ? 'حظر' : 'Block'}</button>
            </div>
          </div>}
          {step === 'block-submitting' && <div className="report-status"><p>{ar ? 'جارٍ الحظر…' : 'Blocking…'}</p></div>}
          {step === 'block-done' && <div className="report-status">
            <p>{ar ? 'تم حظر هذا الحساب.' : 'This account has been blocked.'}</p>
            <button type="button" className="approved-button wide" onClick={() => { setOpen(false); onBlocked?.(); }}>{ar ? 'تم' : 'Done'}</button>
          </div>}
...
function SavedScreen({ ar, saved, lists, activeListId, edits, onSelectList, onCreateList, onRenameList, onDeleteList, onOpen, onUnsave }: { ar: boolean; saved: string[]; lists: SavedList[]; activeListId: string | null; edits: CreatorEdit[]; onSelectList: (id: string | null) => void; onCreateList: () => void; onRenameList: (id: string, name: string) => Promise<void>; onDeleteList: (id: string) => Promise<void>; onOpen: (edit: CreatorEdit) => void; onUnsave: (id: string) => void }) {
  ...
  return <SimpleScreen kicker={ar ? 'مكتبتك' : 'Your library'} title={ar ? 'المحفوظات' : 'Saved'}>
    <p>{ar ? 'كل ما تحفظه، منظماً بطريقتك.' : 'Everything you save, organised your way.'}</p>
    <div className="saved-list-pills" aria-label={ar ? 'قوائم المحفوظات' : 'Saved lists'}>
      <button className={!activeListId ? 'active' : ''} onClick={() => onSelectList(null)}>{ar ? 'كل المحفوظات' : 'All Saved'}</button>
...
function SavedGridCard({ edit, ar, onOpen, onUnsave }: { edit: CreatorEdit; ar: boolean; onOpen: () => void; onUnsave: () => void }) {
  ...
  return <article className="saved-grid-card" data-testid={`saved-grid-${edit.id}`}>
    ...
    <div className="saved-grid-copy">
      <button className="saved-grid-caption" onClick={onOpen}>{caption}</button>
      <div><span>{edit.creatorName || edit.creatorUsername || (ar ? 'منشئ TASTEKIN' : 'TASTEKIN creator')}</span><button className="approved-save saved" onClick={onUnsave} aria-label={ar ? 'إزالة من المحفوظات' : 'Remove from saved'}><Bookmark size={17} fill="currentColor" /></button></div>
    </div>
  </article>;
}

function CreateSavedListDrawer({ ar, open, onClose, onCreate }: { ar: boolean; open: boolean; onClose: () => void; onCreate: (name: string) => Promise<void> }) {
  const [name, setName] = useState('');
  const [busy, setBusy] = useState(false);
  return <Drawer.Root open={open} onOpenChange={(next) => { if (!next) { setName(''); onClose(); } }}>
    <Drawer.Portal><Drawer.Overlay className="approved-drawer-overlay" /><Drawer.Content className="approved-drawer-content saved-list-sheet" aria-label={ar ? 'إنشاء قائمة محفوظات' : 'Create Saved list'}>
      <label>{ar ? 'اسم القائمة' : 'List name'}<input autoFocus value={name} maxLength={60} onChange={(event) => setName(event.target.value)} /></label>
      <button className="approved-button primary wide" disabled={busy || !name.trim()} onClick={() => { setBusy(true); void onCreate(name.trim()).then(() => { setName(''); onClose(); }).catch((error) => window.alert(error instanceof Error ? error.message : String(error))).finally(() => setBusy(false)); }}>{ar ? 'إنشاء' : 'Create'}</button>
    </Drawer.Content></Drawer.Portal>
  </Drawer.Root>;
}

function SavedListPicker({ ar, editId, lists, onClose, onToggle }: { ar: boolean; editId: string | null; lists: SavedList[]; onClose: () => void; onToggle: (listId: string, editId: string, active: boolean) => void }) {
  return <Drawer.Root open={Boolean(editId)} onOpenChange={(next) => { if (!next) onClose(); }}>
    <Drawer.Portal><Drawer.Overlay className="approved-drawer-overlay" /><Drawer.Content className="approved-drawer-content saved-list-sheet" aria-label={ar ? 'إضافة إلى القوائم' : 'Add to lists'}>
      <h2>{ar ? 'إضافة إلى قائمة' : 'Add to a list'}</h2>
      <div className="saved-list-options">{lists.map((list) => <label key={list.id}><input type="checkbox" checked={Boolean(editId && list.editIds.includes(editId))} onChange={(event) => { if (editId) onToggle(list.id, editId, event.target.checked); }} />{list.name}</label>)}</div>
      <button className="approved-button primary wide" onClick={onClose}>{ar ? 'تم' : 'Done'}</button>
    </Drawer.Content></Drawer.Portal>
  </Drawer.Root>;
}
...
function InboxScreen({ ar, activeConversationId, onOpen, onSignIn }: { ar: boolean; activeConversationId: string | null; onOpen: (id: string) => void; onSignIn: () => void }) {
  const session = useTasteSession();
  ...
  if (session.status !== 'authenticated') return <SimpleScreen kicker={ar ? 'الرسائل' : 'Messages'} title={ar ? 'صندوق الوارد' : 'Inbox'}><div className="workspace-notice">{ar ? 'سجّل الدخول لرؤية رسائلك.' : 'Sign in to view your messages.'}<button onClick={onSignIn}>{ar ? 'تسجيل الدخول' : 'Sign in'}</button></div></SimpleScreen>;
...
function AuthScreen({ ar, initialResetToken, initialError, onDone }: { ar: boolean; initialResetToken: string | null; initialError: string | null; onDone: () => void }) {
  const session = useTasteSession();
  const t = (en: string, arabic: string) => ar ? arabic : en;
  const [mode, setMode] = useState<'signin' | 'signup' | 'forgot' | 'reset'>(initialResetToken ? 'reset' : 'signin');
  ...
  const submitEmailPassword = async (event: FormEvent) => {
    ...
    setError(''); setNotice('');
    if (!email.trim() || !email.includes('@')) { setError(t('Enter a valid email address.', 'أدخل بريداً إلكترونياً صحيحاً.')); return; }
  ...
  const submitReset = async (event: FormEvent) => {
    ...
    setBusy(true);
    try {
      const response = await fetch('/api/auth/reset-password', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: initialResetToken, password }) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) { setError(data.error || t('This reset link is invalid or has expired.', 'رابط إعادة التعيين غير صالح أو منتهي الصلاحية.')); return; }
      setNotice(data.message || t('Your password has been reset. You can now sign in.', 'تمت إعادة تعيين كلمة المرور. يمكنك تسجيل الدخول الآن.'));
  ...
  return <SimpleScreen kicker={t('Account', 'الحساب')} title={mode === 'signup' ? t('Create your account', 'أنشئ حسابك') : mode === 'forgot' ? t('Reset your password', 'إعادة تعيين كلمة المرور') : mode === 'reset' ? t('Choose a new password', 'اختر كلمة مرور جديدة') : t('Sign in', 'تسجيل الدخول')}>
    ...
    {(mode === 'signin' || mode === 'signup') && <form onSubmit={(event) => void submitEmailPassword(event)}>
      ...
      <div className="auth-links">
        {mode === 'signin' ? (
          <button type="button" className="auth-link" onClick={() => { setMode('signup'); setError(''); }}>{t("Don't have an account? Sign up", 'ليس لديك حساب؟ أنشئ حساباً')}</button>
        ) : (
          <button type="button" className="auth-link" onClick={() => { setMode('signin'); setError(''); }}>{t('Already have an account? Sign in', 'لديك حساب بالفعل؟ سجّل الدخول')}</button>
        )}
...
type BlockedAccountRow = { id: string; username: string | null; displayName: string | null; avatar: string | null; createdAt: string };

/**
 * Lists accounts the signed-in user has blocked and lets them unblock.
 * Unblocking restores visibility and interaction but never re-creates a
 * prior follow relationship — that is a deliberate, server-enforced choice.
 */
function BlockedAccountsScreen({ ar, onSignIn }: { ar: boolean; onSignIn: () => void }) {
  const session = useTasteSession();
  ...
  const load = useCallback(async () => {
    ...
    setState('loading'); setError('');
    try {
      const response = await fetch('/api/blocks', { credentials: 'include', cache: 'no-store' });
      if (!response.ok) throw new Error(await describeFailedResponse(response));
      const payload = await response.json() as { blocks: BlockedAccountRow[] };
      setRows(payload.blocks); setState('ready');
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err)); setState('error');
    }
  }, [session.status]);
  ...
  const unblock = async (row: BlockedAccountRow) => {
    if (!row.username) return;
    setUnblockingId(row.id);
    try {
      const response = await fetch(`/api/blocks/${encodeURIComponent(row.username)}`, { method: 'DELETE', credentials: 'include' });
      if (!response.ok) throw new Error(await describeFailedResponse(response));
  ...
  if (session.status !== 'authenticated') {
    return <SimpleScreen kicker={ar ? 'الخصوصية والأمان' : 'Privacy & Safety'} title={ar ? 'الحسابات المحظورة' : 'Blocked accounts'}>
      <div className="workspace-notice">{ar ? 'سجّل الدخول لعرض الحسابات المحظورة.' : 'Sign in to view blocked accounts.'}<button onClick={onSignIn}>{ar ? 'تسجيل الدخول' : 'Sign in'}</button></div>
    </SimpleScreen>;
  }
  ...
  return <SimpleScreen kicker={ar ? 'الخصوصية والأمان' : 'Privacy & Safety'} title={ar ? 'الحسابات المحظورة' : 'Blocked accounts'}>
    ...
    {state === 'ready' && rows.length > 0 && <div className="admin-app-list">{rows.map((row) => <div key={row.id} className="admin-app-row">
      ...
      <button className="approved-button" disabled={unblockingId === row.id} onClick={() => void unblock(row)}>{unblockingId === row.id ? (ar ? 'جارٍ…' : 'Working…') : (ar ? 'إلغاء الحظر' : 'Unblock')}</button>
    </div>)}</div>}
  </SimpleScreen>;
}
...
/**
 * The active day's stops only — never connects every venue across every
 * trip day (that produced a dense, unreadable graph of overlapping labels
 * and lines). Positions are a simple client-side normalization of the
 * real lat/lng values Google Places already returned — never a Maps JS
 * embed or a fetched map tile, so no browser-restricted key is needed,
 * and the account's only Google Maps key is scoped to Places API New +
 * Routes API, not Maps JavaScript/Embed (see GOOGLE_MAPS_API_KEY in
 * TASTEKIN_CLAUDE.md §7) — exposing it to the browser for a JS map would
 * both leak a server key and likely fail outright (wrong API scope).
 * No route line is drawn: Google Routes here only ever returns distance
 * and duration, never a polyline, so a connecting line would not be a
 * geographically real route — numbered markers alone are honest. A venue
 * missing coordinates is simply omitted from the map (with a concise
 * notice) rather than breaking the layout.
 */
function KinTravelDayMap({ places, ar, destination }: { places: KinTravelPlace[]; ar: boolean; destination: string }) {
  const t = (en: string, arabic: string) => ar ? arabic : en;
  const [selectedPlaceId, setSelectedPlaceId] = useState<string | null>(null);
  const popoverRef = useRef<HTMLDivElement | null>(null);
  // The popover can appear near the bottom of a short day's map card, right
  // where the fixed bottom nav sits. scrollIntoView's own "nearest" mode
  // doesn't know a fixed-position nav is covering part of the viewport
  // (scroll-margin has no effect on it in practice here), so this measures
  // the actual overlap against the real nav element and scrolls exactly
  // enough to clear it whenever a marker is newly selected.
  useLayoutEffect(() => {
    ...
    const overlap = popover.getBoundingClientRect().bottom - limit;
    // behavior: 'instant' bypasses the app's global `html { scroll-behavior:
    // smooth }` — this correction must land before the very next paint, not
    // animate over the next few hundred ms leaving the popover behind the
    // nav in the meantime.
    if (overlap > 0) window.scrollBy({ top: overlap + 12, left: 0, behavior: 'instant' });
  }, [selectedPlaceId]);
  const numbered = places.map((place, index) => ({ place, number: index + 1 }));
  const withCoords = numbered.filter((item): item is { place: KinTravelPlace & { lat: number; lng: number }; number: number } => item.place.lat !== null && item.place.lng !== null);
  const missingCount = numbered.length - withCoords.length;

  if (withCoords.length === 0) {
    return <div className="kin-map-empty" data-testid="kin-map-empty">
      <MapPin size={20} aria-hidden="true" />
      <span>{t("KIN couldn't place this day's stops on a map yet.", 'تعذّر على كين وضع محطات هذا اليوم على الخريطة بعد.')}</span>
    </div>;
  }

  const lats = withCoords.map((item) => item.place.lat);
  const lngs = withCoords.map((item) => item.place.lng);
  const minLat = Math.min(...lats); const maxLat = Math.max(...lats);
  const minLng = Math.min(...lngs); const maxLng = Math.max(...lngs);
  const pad = 12;
  const positioned = withCoords.map((item) => ({
    ...item,
    left: pad + (maxLng > minLng ? (item.place.lng - minLng) / (maxLng - minLng) : 0.5) * (100 - pad * 2),
    top: pad + (1 - (maxLat > minLat ? (item.place.lat - minLat) / (maxLat - minLat) : 0.5)) * (100 - pad * 2),
  }));
  const selected = positioned.find((item) => item.place.placeId === selectedPlaceId) ?? null;

  return <div className="kin-map-wrap">
    <div className="kin-map" data-testid="kin-travel-map" role="group" aria-label={t(`Map of today's stops in ${destination}`, `خريطة محطات اليوم في ${destination}`)}>
      {positioned.map((item) => <button type="button" key={item.place.placeId} data-testid="kin-map-pin"
        className={`kin-map-pin${selectedPlaceId === item.place.placeId ? ' selected' : ''}`}
        style={{ left: `${item.left}%`, top: `${item.top}%` }}
        aria-pressed={selectedPlaceId === item.place.placeId}
        aria-label={`${item.number}. ${item.place.name}`}
        onClick={() => setSelectedPlaceId((current) => current === item.place.placeId ? null : item.place.placeId)}>
        {item.number}
      </button>)}
    </div>
    {missingCount > 0 && <p className="kin-map-notice" role="status" data-testid="kin-map-missing-notice">
      {t(missingCount === 1 ? "1 place from this day couldn't be shown on the map." : `${missingCount} places from this day couldn't be shown on the map.`,
        missingCount === 1 ? 'تعذّر عرض مكان واحد من هذا اليوم على الخريطة.' : `تعذّر عرض ${missingCount} أماكن من هذا اليوم على الخريطة.`)}
    </p>}
    {selected && <div ref={popoverRef} className="kin-map-popover" data-testid="kin-map-popover">
      <button type="button" className="kin-map-popover-close" aria-label={t('Close', 'إغلاق')} onClick={() => setSelectedPlaceId(null)}><X size={14} /></button>
      <span className="kin-map-popover-index" aria-hidden="true">{selected.number}</span>
      <div className="kin-map-popover-body">
        <strong>{selected.place.name}</strong>
        {selected.place.mapsUrl && <a href={selected.place.mapsUrl} target="_blank" rel="noopener noreferrer">{t('Open in Google Maps', 'افتح في خرائط Google')}</a>}
      </div>
    </div>}
  </div>;
}
...
function FormattedText({ text }: { text: string }) {
  ...
  return <>{text.split('\n').map((line, i) => {
    ...
    const elements = content.split(/(\*\*.*?\*\*)/g).map((part, j) => (
      part.startsWith('**') && part.endsWith('**') ? <strong key={j}>{part.slice(2, -2)}</strong> : part
    ));
    return <Fragment key={i}>
      {i > 0 && !isHeading && <br />}
      {isHeading ? <h3 className="kin-markdown-heading">{elements}</h3> : elements}
    </Fragment>;
  })}</>;
}

const KIN_SLOT_LABELS: Record<string, { en: string; ar: string }> = {
...
function KinScreen({ ar, stylingItemIds, onClearStylingItems, onChangeStylingItems, onUnavailable }: { ar: boolean; stylingItemIds: Set<string>; onClearStylingItems: () => void; onChangeStylingItems: () => void; onUnavailable: () => void }) {
  ...
  // whatever scroll position the form happened to be at (e.g. having
  ...
  if (view === 'travel-overview' && travelPlan) {
    ...
    return <section data-testid="kin-screen">
      ...
      <h1 className="kin-headline" dir="auto">{t(`${displayDestination}, shaped around you.`, `${displayDestination}، مصمم من أجلك.`)}</h1>
      ...
      <div className="kin-hero" data-testid="kin-travel-hero">
        ...
        <span className="kin-hero-tag" data-testid="kin-hero-tag">
          {showDayNumber ? t(`Day ${selectedDayIndex + 1} · ${displayDestination}`, `اليوم ${selectedDayIndex + 1} · ${displayDestination}`) : displayDestination}
        </span>
      </div>
      ...
      {planView === 'map' ? <div className="kin-day-card" style={{ marginBottom: 24 }}>
        {!activeDay || activeDay.places.length === 0
          ...
          : <KinTravelDayMap key={`map-${activeDay.dayIndex}`} places={activeDay.places} ar={ar} destination={displayDestination} />}
      </div> : activeDay && <div className="kin-day-card" data-testid="kin-day-preview" style={{ marginBottom: 24 }}>
        <span className="kin-day-kicker">{(activeDay.date ? new Date(`${activeDay.date}T00:00:00Z`).toLocaleDateString(ar ? 'ar' : 'en-US', { weekday: 'long', timeZone: 'UTC' }) : `${t('Day', 'اليوم')} ${activeDay.dayIndex + 1}`)} · {displayDestination}</span>
...
/**
 * Edit Item. The photo is fixed — there is no file input here and this
 * screen never calls POST /closet-items/media. PUT /closet-items/:id is a
 * full replace, so every organized field is resent on every save,
 * including the ones the user didn't touch; an optional field the user
 * cleared (or left empty) is sent as explicit null, never omitted, and
 * confirmationStatus is never included so the item's confirmed/
 * pending_review state is left exactly as it was.
 */
function EditClosetItemScreen({ ar, item, onDone, onUnavailable }: { ar: boolean; item: ClosetItem; onDone: () => void; onUnavailable: () => void }) {
  const session = useTasteSession();
  const t = (en: string, arabic: string) => ar ? arabic : en;
  ...
  if (!allowed) return <SimpleScreen kicker={t('My Things', 'أغراضي')} title={t('Edit item', 'تعديل الغرض')}><Empty text={t('Loading…', 'جارٍ التحميل…')} /></SimpleScreen>;

  const requiredFieldsChosen = Boolean(itemType && primaryColor);
  const saveDisabled = saving || !requiredFieldsChosen;

  const save = async () => {
    if (saving) return;
    setSaving(true); setSubmitError('');
    try {
      const response = await fetch(`/api/closet-items/${encodeURIComponent(item.id)}`, {
        method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          itemType, primaryColor,
          style: style || null,
          ...
          season: season || null,
          brand: brand.trim() ? brand.trim().slice(0, CLOSET_MAX_BRAND_LENGTH) : null,
        }),
      });
      if (!response.ok) throw new Error(await describeFailedResponse(response));
      onDone();
  ...
  return <SimpleScreen kicker={t('My Things', 'أغراضي')} title={t('Edit item', 'تعديل الغرض')}>
    ...
    <ClosetChoiceField label={t('Primary color', 'اللون الأساسي')} options={CLOSET_PRIMARY_COLORS} value={primaryColor} onSelect={setPrimaryColor} disabled={saving} />

    <details className="nested-details"><summary>{t('Optional details', 'تفاصيل اختيارية')}</summary><div className="details-body">
      <ClosetChoiceField label={t('Style', 'الطراز')} options={CLOSET_STYLES} value={style} onSelect={(next) => setStyle(next === style ? '' : next)} disabled={saving} />
      <ClosetChoiceField label={t('Occasion', 'المناسبة')} options={CLOSET_OCCASIONS} value={occasion} onSelect={(next) => setOccasion(next === occasion ? '' : next)} disabled={saving} />
      <ClosetChoiceField label={t('Season', 'الموسم')} options={CLOSET_SEASONS} value={season} onSelect={(next) => setSeason(next === season ? '' : next)} disabled={saving} />
      <label className="form-field"><span>{t('Brand', 'العلامة التجارية')}</span><input type="text" value={brand} onChange={(event) => setBrand(event.target.value.slice(0, CLOSET_MAX_BRAND_LENGTH))} disabled={saving} placeholder={t('Optional', 'اختياري')} /></label>
    </div></details>

    {submitError && <p className="workspace-notice" role="alert" data-testid="my-things-edit-error">{submitError}</p>}

    <button className="approved-button primary wide" style={{ marginTop: 12 }} data-testid="my-things-edit-save" onClick={() => void save()} disabled={saveDisabled}>{saving ? t('Saving…', 'جارٍ الحفظ…') : t('Save changes', 'حفظ التغييرات')}</button>
  </SimpleScreen>;
}

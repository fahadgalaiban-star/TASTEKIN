...
export default function App() {
  ...
  const go = (next: Screen) => {
    ...
    if (next !== 'profile') setVisitorPreview(false);
  ...
  return <div className="approved-app" dir={ar ? 'rtl' : 'ltr'}><main className="approved-shell">
    ...
    {screen === 'you' && <SimpleScreen kicker={owner ? t('Creator owner mode', 'وضع مالك الحساب') : t('Your account', 'حسابك')} title={owner ? t('Your profile', 'ملفك الشخصي') : t('Alex Morgan', 'أليكس مورغان')}><div className="approved-panel identity"><Avatar /><div><strong>{owner ? t('Fheed Alaiban', 'فهيد العيبان') : t('Alex Morgan', 'أليكس مورغان')}</strong><span>{owner ? 'Kuwait City, Kuwait' : '@alexmorgan'}</span></div></div><div className="approved-panel"><h3>{t('Taste profile', 'ملف الذوق')}</h3><p>{t('Fashion & outfits, travel, places, restaurants, and daily routines.', 'أزياء وإطلالات، سفر، أماكن، مطاعم، وروتين يومي.')}</p></div>{owner && <button className="approved-button wide" onClick={() => go('profile')}>{t('View profile', 'عرض الملف')}</button>}</SimpleScreen>}
     {screen === 'profile' && <Profile ar={ar} owner={owner} visitorPreview={visitorPreview} following={following} subscribed={subscribed} edits={published} onViewAsVisitor={() => setVisitorPreview(true)} onExitVisitor={() => setVisitorPreview(false)} onFollow={() => { if (!owner) { setFollowing(!following); write('following-fheed', !following); } }} onSubscribe={() => { if (!owner) go('subscribe'); }} onEdit={openEdit} onCollections={() => go('collections')} onAbout={() => go('about')} onMatch={() => go('match')} />}
...
function Profile({ ar, owner, visitorPreview, following, subscribed, edits, onViewAsVisitor, onExitVisitor, onFollow, onSubscribe, onEdit, onCollections, onAbout, onMatch }: { ar: boolean; owner: boolean; visitorPreview: boolean; following: boolean; subscribed: boolean; edits: CreatorEdit[]; onViewAsVisitor: () => void; onExitVisitor: () => void; onFollow: () => void; onSubscribe: () => void; onEdit: (edit: CreatorEdit) => void; onCollections: () => void; onAbout: () => void; onMatch: () => void }) { const ownerView = owner && !visitorPreview; return <section><div className="approved-profile-head"><Avatar /><div><div className="approved-name"><h1>{ar ? 'فهيد العيبان' : 'Fheed Alaiban'}</h1><Check size={15} /></div><span><bdi dir="ltr">@fheed</bdi></span><p>{ar ? 'مدينة الكويت، الكويت · أزياء وإطلالات · سفر · أماكن' : 'Kuwait City, Kuwait · Fashion & Outfits · Travel · Places'}</p></div></div><button className="approved-match" onClick={onMatch}>{ar ? 'تطابق ذوق ٩٢٪' : '92% Taste Match'}</button><div className="approved-actions">{ownerView ? <><button className="approved-button primary" onClick={() => onEdit(edits[0])}>{ar ? 'تعديل الملف' : 'Edit profile'}</button><button className="approved-button" onClick={onViewAsVisitor}>{ar ? 'عرض كزائر' : 'View as visitor'}</button></> : <><button className="approved-button" onClick={onFollow} disabled={visitorPreview}>{following ? (ar ? 'تتابع' : 'Following') : (ar ? 'متابعة' : 'Follow')}</button><button className="approved-button primary" onClick={onSubscribe} disabled={visitorPreview}>{subscribed ? (ar ? 'مشترك' : 'Subscribed') : <Price ar={ar} />}</button></>}</div>{visitorPreview && <button className="approved-button wide visitor-exit" onClick={onExitVisitor}>{ar ? 'إنهاء معاينة الزائر' : 'Exit visitor preview'}</button>}<div className="approved-tabs"><button className="active" onClick={() => onEdit(edits[0])}>{ar ? 'التعديلات' : 'Edits'}</button><button onClick={onCollections}>{ar ? 'المجموعات' : 'Collections'}</button><button onClick={onAbout}>{ar ? 'حول' : 'About'}</button></div><div className="approved-grid">{edits.slice(0, 6).map((edit) => <button className="approved-grid-card" key={edit.id} onClick={() => onEdit(edit)}><img style={{ aspectRatio: cropAspectRatio(edit.crop?.aspect, edit.crop) }} src={imageSrc(edit.image)} alt={edit.altText} /><strong>{ar ? edit.titleAr || edit.title : edit.title}</strong></button>)}</div></section>; }

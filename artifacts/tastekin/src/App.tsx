...
function PlaceDetails({ edit, ar, compact = false }: { edit: CreatorEdit; ar: boolean; compact?: boolean }) {
  ...
  return <div className={`place-details ${compact ? 'compact' : ''}`}>
    {edit.placeName && <strong className="place-name">{edit.placeName}</strong>}
...
function EditDetail({ edit, ar, subscribed, saved, onSave, onSubscribe }: { edit: CreatorEdit; ar: boolean; subscribed: boolean; saved: boolean; onSave: () => void; onSubscribe: () => void }) { const locked = edit.access === 'locked'; const caption = publicCaptionLine(edit, ar); const outfitItems = (edit.outfitItems || []).filter((item) => item.type || item.brand || item.name); return <SimpleScreen kicker={locked ? (ar ? 'للمشتركين فقط' : 'Subscribers only') : (ar ? 'تعديل عام' : 'Public Edit')} title={caption}>{edit.image && <div className={`approved-detail-art ${locked ? 'locked' : ''}`} style={{ aspectRatio: cropAspectRatio(edit.crop?.aspect, edit.crop), height: 'auto' }}><img src={imageSrc(edit.image)} alt={edit.altText} />{locked && <div><LockKeyhole size={26} />{caption && <strong>{caption}</strong>}</div>}</div>}{!edit.image && <div className={`place-detail-panel ${locked ? 'locked' : ''}`}>{locked && <span className="approved-access"><LockKeyhole size={11} /> {ar ? 'للمشتركين فقط' : 'Subscribers only'}</span>}<PlaceDetails edit={edit} ar={ar} /></div>}{!edit.placeName && (edit.location || edit.locationAr) && <div className="approved-location"><MapPin size={14} />{placeLocation(edit, ar)}</div>}{locked ? <div className="approved-panel"><h3>{ar ? 'هذا التعديل للمشتركين' : 'This edit is for subscribers'}</h3><p>{ar ? 'تظل الوسائط الخاصة محمية إلى أن يتم تأكيد اشتراكك في حسابك.' : 'Private media stays protected until your subscription is confirmed on your account.'}</p><button className="approved-button primary wide" onClick={onSubscribe}>{subscribed ? (ar ? 'بانتظار تأكيد الاشتراك' : 'Subscription pending confirmation') : <Price ar={ar} />}</button></div> : <>{isPlaceCategory(edit.category) && edit.image && <PlaceDetails edit={edit} ar={ar} />}{edit.showOutfitDetails && outfitItems.length > 0 && <div className="outfit-published"><h3>{ar ? 'تفاصيل الإطلالة' : 'Outfit details'}</h3>{outfitItems.map((item, index) => <div key={index}><strong>{item.type || item.name}</strong><span>{[item.brand, item.name].filter(Boolean).join(' · ')}</span>{item.link && <a href={item.link} target="_blank" rel="noreferrer">{ar ? 'عرض المنتج' : 'View item'}</a>}</div>)}</div>}<button className={`approved-button wide ${saved ? 'primary' : ''}`} onClick={onSave}>{saved ? (ar ? 'تم الحفظ' : 'Saved') : (ar ? 'احفظ هذا التعديل' : 'Save this edit')}</button></>}</SimpleScreen>; }
...
function EditComposer({ ar, form, collections, busy, onChange, onCropPrepared, onBack, onDraft, onPreview, onPublish }: { ar: boolean; form: EditForm; collections: CreatorCollection[]; busy: boolean; onChange: (form: EditForm) => void; onCropPrepared: (crop: PendingCrop) => void; onBack: () => void; onDraft: () => void; onPreview: () => void; onPublish: () => void }) {
  ...
  return <section className="creator-composer">
    ...
    <details className="composer-details" open={placeCategory}><summary>{t('Add details', 'أضف تفاصيل')}</summary><div className="details-body">
      <div className="form-two"><SelectField label={t('Category', 'الفئة')} value={form.category} onChange={(value) => changeCategory(value as Exclude<Category, 'All'>)} options={categories.filter((item) => item.id !== 'All').map((item) => ({ value: item.id, label: ar ? item.ar : item.en }))} /><Field label={t('Location', 'الموقع')} value={ar ? form.locationAr : form.location} onChange={(value) => update(ar ? 'locationAr' : 'location', value)} placeholder="Kuwait City" /></div>

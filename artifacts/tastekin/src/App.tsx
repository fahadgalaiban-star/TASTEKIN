...
type Screen = 'home' | 'explore' | 'add' | 'saved' | 'you' | 'profile' | 'profileEdit' | 'collections' | 'collection' | 'about' | 'match' | 'edit' | 'subscribe' | 'composer' | 'creatorPreview' | 'collectionManager';
...
export default function App() {
  ...
  const commitEdit = async (status: EditStatus) => {
    ...
    const next = { id, ...formToSave, title: formToSave.title.trim() || formToSave.caption.trim().slice(0, 80), titleAr: formToSave.titleAr.trim() || formToSave.caption.trim().slice(0, 80), captionAr: formToSave.captionAr || formToSave.caption, status } as CreatorEdit;
  ...
  return <div className="approved-app" dir={ar ? 'rtl' : 'ltr'}><main className="approved-shell">
    ...
    {screen === 'you' && <SimpleScreen kicker={owner ? t('Creator owner mode', 'وضع مالك الحساب') : t('Your account', 'حسابك')} title={owner ? t('Your profile', 'ملفك الشخصي') : t('Alex Morgan', 'أليكس مورغان')}><div className="approved-panel identity"><Avatar profile={creatorProfile} /><div><strong>{owner ? creatorProfile.displayName : t('Alex Morgan', 'أليكس مورغان')}</strong><span>{owner ? `${creatorProfile.city}, ${creatorProfile.country}` : '@alexmorgan'}</span></div></div><div className="approved-panel"><h3>{t('Taste profile', 'ملف الذوق')}</h3><p>{creatorProfile.interests.join(' · ')}</p></div>{owner && <button className="approved-button wide" onClick={() => go('profile')}>{t('View profile', 'عرض الملف')}</button>}</SimpleScreen>}
     {screen === 'profile' && <Profile ar={ar} owner={owner} visitorPreview={visitorPreview} following={following} subscribed={subscribed} profile={creatorProfile} edits={published} onViewAsVisitor={() => setVisitorPreview(true)} onExitVisitor={() => setVisitorPreview(false)} onFollow={() => { if (!owner) { setFollowing(!following); write('following-fheed', !following); } }} onSubscribe={() => { if (!owner) go('subscribe'); }} onEditProfile={openProfileEditor} onEdit={openEdit} onCollections={() => go('collections')} onAbout={() => go('about')} onMatch={() => go('match')} />}
...
function publicCaptionLine(edit: CreatorEdit, ar: boolean) { return (ar ? edit.captionAr || edit.caption : edit.caption || edit.captionAr).split(/\r?\n/, 1)[0].trim(); }
function EditCard({ edit, ar, saved, onSave, onOpen }: { edit: CreatorEdit; ar: boolean; saved: boolean; onSave: () => void; onOpen: () => void }) { const caption = publicCaptionLine(edit, ar); return <article className="approved-card" data-testid={`edit-card-${edit.id}`}><button className="approved-art" style={{ aspectRatio: cropAspectRatio(edit.crop?.aspect, edit.crop) }} onClick={onOpen}><img src={imageSrc(edit.image)} alt={edit.altText} />{edit.access === 'locked' && <span className="approved-access"><LockKeyhole size={11} /> {ar ? 'للمشتركين فقط' : 'Subscribers only'}</span>}</button>{caption && <div className="approved-caption"><div className="approved-caption-row"><button className="approved-card-title" data-testid={`edit-title-${edit.id}`} onClick={onOpen}>{caption}</button><button className={`approved-save ${saved ? 'saved' : ''}`} data-testid={`save-${edit.id}`} onClick={onSave} aria-label={saved ? (ar ? 'إزالة من المحفوظات' : 'Remove from saved') : (ar ? 'حفظ التعديل' : 'Save Edit')} aria-pressed={saved}><Bookmark size={18} fill={saved ? 'currentColor' : 'none'} /></button></div></div>}{!caption && <div className="approved-caption approved-caption-empty"><button className={`approved-save ${saved ? 'saved' : ''}`} data-testid={`save-${edit.id}`} onClick={onSave} aria-label={saved ? (ar ? 'إزالة من المحفوظات' : 'Remove from saved') : (ar ? 'حفظ التعديل' : 'Save Edit')} aria-pressed={saved}><Bookmark size={18} fill={saved ? 'currentColor' : 'none'} /></button></div>}</article>; }
function EditDetail({ edit, ar, subscribed, saved, onSave, onSubscribe }: { edit: CreatorEdit; ar: boolean; subscribed: boolean; saved: boolean; onSave: () => void; onSubscribe: () => void }) { const locked = edit.access === 'locked'; const caption = publicCaptionLine(edit, ar); const outfitItems = (edit.outfitItems || []).filter((item) => item.type || item.brand || item.name); return <SimpleScreen kicker={locked ? (ar ? 'للمشتركين فقط' : 'Subscribers only') : (ar ? 'تعديل عام' : 'Public Edit')} title={caption}><div className={`approved-detail-art ${locked ? 'locked' : ''}`} style={{ aspectRatio: cropAspectRatio(edit.crop?.aspect, edit.crop), height: 'auto' }}><img src={imageSrc(edit.image)} alt={edit.altText} />{locked && <div><LockKeyhole size={26} />{caption && <strong>{caption}</strong>}</div>}</div>{(edit.location || edit.locationAr) && <div className="approved-location"><MapPin size={14} />{ar ? edit.locationAr || edit.location : edit.location}</div>}{locked ? <div className="approved-panel"><h3>{ar ? 'هذا التعديل للمشتركين' : 'This edit is for subscribers'}</h3><p>{ar ? 'تظل الوسائط الخاصة محمية إلى أن يتم تأكيد اشتراكك في حسابك.' : 'Private media stays protected until your subscription is confirmed on your account.'}</p><button className="approved-button primary wide" onClick={onSubscribe}>{subscribed ? (ar ? 'بانتظار تأكيد الاشتراك' : 'Subscription pending confirmation') : <Price ar={ar} />}</button></div> : <>{edit.showOutfitDetails && outfitItems.length > 0 && <div className="outfit-published"><h3>{ar ? 'تفاصيل الإطلالة' : 'Outfit details'}</h3>{outfitItems.map((item, index) => <div key={index}><strong>{item.type || item.name}</strong><span>{[item.brand, item.name].filter(Boolean).join(' · ')}</span>{item.link && <a href={item.link} target="_blank" rel="noreferrer">{ar ? 'عرض المنتج' : 'View item'}</a>}</div>)}</div>}<button className={`approved-button wide ${saved ? 'primary' : ''}`} onClick={onSave}>{saved ? (ar ? 'تم الحفظ' : 'Saved') : (ar ? 'احفظ هذا التعديل' : 'Save this edit')}</button></>}</SimpleScreen>; }
function Profile({ ar, owner, visitorPreview, following, subscribed, profile, edits, onViewAsVisitor, onExitVisitor, onFollow, onSubscribe, onEditProfile, onEdit, onCollections, onAbout, onMatch }: { ar: boolean; owner: boolean; visitorPreview: boolean; following: boolean; subscribed: boolean; profile: CreatorProfile; edits: CreatorEdit[]; onViewAsVisitor: () => void; onExitVisitor: () => void; onFollow: () => void; onSubscribe: () => void; onEditProfile: () => void; onEdit: (edit: CreatorEdit) => void; onCollections: () => void; onAbout: () => void; onMatch: () => void }) {
  ...
  const locationAndInterests = [`${profile.city}, ${profile.country}`, profile.interests.join(' · '), profile.age ? (ar ? `العمر ${profile.age}` : `Age ${profile.age}`) : ''].filter(Boolean).join(' · ');
  return <section><div className="approved-profile-head"><Avatar profile={profile} /><div><div className="approved-name"><h1>{profile.displayName}</h1>{profile.verified && <button className="taste-seal" type="button" aria-label="Verified by TASTEKIN" aria-expanded={sealOpen} onClick={() => setSealOpen(!sealOpen)}><img src={TASTE_SEAL_IMAGE} alt="" /></button>}</div><span><bdi dir="ltr">@{profile.username}</bdi></span><p>{locationAndInterests}</p></div></div>{sealOpen && <div className="taste-seal-popover" role="dialog" aria-label="Taste Seal verification"><p>Verified by TASTEKIN — selected for authentic taste and identity.</p><button className="approved-icon" onClick={() => setSealOpen(false)} aria-label={ar ? 'إغلاق' : 'Close'}><X size={16} /></button></div>}<button className="approved-match" onClick={onMatch}>{ar ? 'تطابق ذوق ٩٢٪' : '92% Taste Match'}</button><div className="approved-actions">{ownerView ? <><button className="approved-button primary" onClick={onEditProfile}>{ar ? 'تعديل الملف' : 'Edit profile'}</button><button className="approved-button" onClick={onViewAsVisitor}>{ar ? 'عرض كزائر' : 'View as visitor'}</button></> : <><button className="approved-button" onClick={onFollow} disabled={visitorPreview}>{following ? (ar ? 'تتابع' : 'Following') : (ar ? 'متابعة' : 'Follow')}</button><button className="approved-button primary" onClick={onSubscribe} disabled={visitorPreview}>{subscribed ? (ar ? 'مشترك' : 'Subscribed') : <Price ar={ar} />}</button></>}</div>{visitorPreview && <button className="approved-button wide visitor-exit" onClick={onExitVisitor}>{ar ? 'إنهاء معاينة الزائر' : 'Exit visitor preview'}</button>}<div className="approved-tabs"><button className="active" onClick={() => edits[0] && onEdit(edits[0])}>{ar ? 'التعديلات' : 'Edits'}</button><button onClick={onCollections}>{ar ? 'المجموعات' : 'Collections'}</button><button onClick={onAbout}>{ar ? 'حول' : 'About'}</button></div><div className="approved-grid">{edits.slice(0, 6).map((edit) => { const caption = publicCaptionLine(edit, ar); return <button className="approved-grid-card" key={edit.id} onClick={() => onEdit(edit)}><img style={{ aspectRatio: cropAspectRatio(edit.crop?.aspect, edit.crop) }} src={imageSrc(edit.image)} alt={edit.altText} />{caption && <strong>{caption}</strong>}</button>; })}</div></section>;
}

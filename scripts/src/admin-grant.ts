...
// on its own. Safe to re-run: it always reports the exact row it resolved to
// before touching anything, and makes no change unless --yes is passed.
//
// Usage:
...
async function main() {
  ...
  const target = await resolveAdminTarget(args, { allowFromEnv: true });
